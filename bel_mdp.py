...
class belief_mdp:
	...
	def bel_update(self, belief, action_ind, obs_ind):
		...
		new_bel = self.obs_prob(obs_ind,np.arange(len(self.states)),action_ind)*np.dot(self.transition(np.arange(len(self.states)),np.arange(len(self.states)),action_ind), belief)
	...
	def bel_sampler(self, belief, action_ind, state_ind):
		if state_ind != 1:
			...
			obs_allind = np.arange(len(self.observe))
			obs = pyro.sample('obs', dist.Categorical(Variable(torch.FloatTensor(self.obs_prob(obs_allind,state_ind,action_ind))), self.observe))
